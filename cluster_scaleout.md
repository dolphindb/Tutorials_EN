...
- [Scale out a DolphinDB Cluster](#scale-out-a-dolphindb-cluster)
  ...
  - [2. Add Data Nodes](#2-add-data-nodes)
    ...
    - [2.2 Add a Data Node Online](#22-add-a-data-node-online)
    - [2.3 Verification](#23-verification)
  - [3. Add Hard Disks to a Data Node](#3-add-hard-disks-to-a-data-node)

## 1. About DolphinDB Cluster

A DolphinDB cluster has 4 types of nodes: data node, compute node, agent and controller.
- A controller manages the metadata of a cluster and provides the web-based cluster manager. 
  A controller has the following 3 configuration files:
    ...
    - cluster.nodes: a list of nodes in the cluster. The controller uses this file to get information of the cluster nodes.
...
### 2.1 Add a Data Node Offline

#### 2.1.1 Configure the Agent
...
### 2.2 Add a Data Node Online

The modification on configurations when adding a data node online is the same as described in [Section 2.1](#21-add-a-data-node-offline). After configuring agent.cfg and cluster.nodes, please start the agent, connect to the controller via web interface or GUI, and execute the following code:
